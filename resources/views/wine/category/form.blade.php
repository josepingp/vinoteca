                ...
                <form action="{{ $action }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" id="description" rows="4"
                            class="w-full px-3 py-2 leading-tight shadow appearance-none border rounded focus:shadow-outline  text-gray-700 focus:outline-none">{{ old('description', $category->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </div>
